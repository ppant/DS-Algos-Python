...
- [Anagram Check (Manual)](Arrays/AnagramCheckManualSol.py) - $O(n)$
- [Anagram Check (Sorted)](Arrays/AnagramCheckSortedSol.py) - $O(n \log n)$
- [Array Pair Sum](Arrays/ArrayPairSumSol.py): Find pairs that sum to $k$ - $O(n)$
- [Find Missing Element (XOR)](Arrays/ArrayFindTheMissingElementXORSol.py) - $O(n)$
- [Find Missing Element (Brute Force)](Arrays/ArrayFindTheMissingElementBruteForceSol.py) - $O(n \log n)$
- [Find Missing Element (Hash Table)](Arrays/ArrayFindTheMissingElementHashTableSol.py) - $O(n)$
- [Find Missing Element (Sum)](Arrays/ArrayFindTheMissingElementSumSubtractSol.py) - $O(n)$
...
- [Queue Implementation](Queues/QueueImple.py): Basic operations (enqueue, dequeue) - $O(n)$ dequeue
...
- [BST Validation (Solution 1)](Trees/BinarySearchTreeCheckImpleSol1.py) & [Solution 2](Trees/BinarySearchTreeCheckImpleSol2.py) - $O(n)$
- [Binary Search (Iterative)](Trees/BinarySearchImple.py) & [Recursive](Trees/BinarySearchRecursiveImple.py) - $O(\log n)$
...
- [Tree Representations (Nodes & References)](Trees/TreeRepresentationWithNodesReferences.py) & [(List of Lists)](Trees/BuildTreeTest.py)
...
- [Fibonacci (Iterative)](Recursion/FibonacciSeqIterative.py) - $O(n)$
- [Fibonacci (Recursive)](Recursion/FibonacciSeqRecursion.py) - $O(2^n)$
- [Fibonacci (DP)](Recursion/FibonacciSeqDynamic.py) - $O(n)$
- [Coin Change (Recursive)](Recursion/CoinChangeProblemRecursion.py) - $O(2^n)$
- [Coin Change (DP)](Recursion/CoinChangeProblemDynamic.py) - $O(n \times m)$
...
- [Sum of Digits](Recursion/RecursionSumOfDigits.py) - $O(\log_{10} n)$
- [Word Split](Recursion/RecursionWordSplit.py) - $O(n^2)$

### Graph Algorithms 🗺️
Algorithms for graph traversal and pathfinding.
- [Adjacency List](GraphAlgorithms/AdjacencyListGraphImple.py): Graph ADT implementation - $O(1)$ average
...
- [Knight's Tour (Graph Generation)](GraphAlgorithms/TheKnightsTourProblem.py) & [DFS Solution](GraphAlgorithms/DFSImpleTheKnightsTourProblem.py) - $O(k^N)$
- [Word Ladder Problem](GraphAlgorithms/WordLadderProblem.py): Building the word ladder graph - $O(V + E)$
...
- [ ] Include detailed space complexity analysis for each solution
- [ ] Add interactive examples/visualizations
- [ ] Create a difficulty level classification
- [ ] Add more unit tests using `pytest`
