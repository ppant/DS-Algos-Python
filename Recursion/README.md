...
- [Coin Change (Dynamic Programming)](CoinChangeProblemDynamic.py): Optimized solution to the coin change problem using dynamic programming. Complexity: $O(n \times m)$ where $n$ is the amount and $m$ is the number of coins.
...
- [Word Split](RecursionWordSplit.py): Determines if a string can be split into words from a given list. Complexity: $O(n^2)$.
