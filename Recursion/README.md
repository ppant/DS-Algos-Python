...
- [Coin Change (Dynamic Programming)](CoinChangeProblemDynamic.py): Optimized solution to the coin change problem using dynamic programming. Time Complexity: $O(n \cdot m)$ where $n$ is the target sum and $m$ is the number of coins.
...
- [Word Split](RecursionWordSplit.py): Determines if a string can be split into words from a given list using dynamic programming.
