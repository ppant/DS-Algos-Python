...
- [Coin Change (Dynamic Programming)](CoinChangeProblemDynamic.py): Optimized solution to the coin change problem using dynamic programming. Time Complexity: $O(n \cdot m)$ where $n$ is the target amount and $m$ is the number of coin denominations.
...
- [Word Split](RecursionWordSplit.py): Determines if a string can be split into words from a given list. Time Complexity: $O(n^2)$ with DP/memoization.
